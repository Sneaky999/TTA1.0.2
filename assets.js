...
async function initSprites(){
  // ── TILE TEXTURES ──
  // Procedural fallbacks run first (synchronous), then PNG images
  // override them once loaded. Game always has working tiles.
  ...
  const loads=[
    // ── Tile PNGs — override procedural fallbacks when available ──
    // grass.png replaces procedural grass
    loadImg('grass.png').then(i=>{ if(i&&i.naturalWidth>0) SP.tiles[0]=i; }),
    // road_straight.png replaces procedural road
    loadImg('road_straight.png').then(i=>{ if(i&&i.naturalWidth>0) SP.tiles[1]=i; }),
    // intersection.png replaces procedural sidewalk; also stored as SP.tiles.inter
    loadImg('intersection.png').then(i=>{ if(i&&i.naturalWidth>0){ SP.tiles[3]=i; SP.tiles.inter=i; } }),
    // crosswalk.png stored separately — drawn as overlay on road tiles near sidewalk
    loadImg('crosswalk.png').then(i=>{ if(i&&i.naturalWidth>0) SP.tiles.crosswalk=i; }),
    // ── Character SVGs ──

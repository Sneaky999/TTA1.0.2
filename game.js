...
function render(){
  ...
  for(let ty=ty0;ty<=ty1;ty++)for(let tx=tx0;tx<=tx1;tx++){
    ...
    if(type===1){ // Road
      // Draw base road tile (PNG if loaded, else procedural canvas)
      if(SP.tiles[1]) CX.drawImage(SP.tiles[1],sx,sy,T,T);

      // Orientation check — only when tile arrays are in-bounds
      const _rH=ty>=0&&ty<WH&&tx>=0&&tx<WW;
      if(_rH){
        const _rN=ty>0   &&WD[ty-1]&&WD[ty-1][tx]===1;
        const _rS=ty<WH-1&&WD[ty+1]&&WD[ty+1][tx]===1;
        const _rW=tx>0   &&WD[ty][tx-1]===1;
        const _rE=tx<WW-1&&WD[ty][tx+1]===1;
        const _isInter=(_rN||_rS)&&(_rW||_rE);
        const _adjSW=(ty>0   &&WD[ty-1]&&WD[ty-1][tx]===3)||
                     (ty<WH-1&&WD[ty+1]&&WD[ty+1][tx]===3)||
                     (tx>0   &&WD[ty][tx-1]===3)||
                     (tx<WW-1&&WD[ty][tx+1]===3);

        // Overlay intersection texture on road crossings
        if(_isInter && SP.tiles.inter){
          CX.globalAlpha=0.75;
          CX.drawImage(SP.tiles.inter,sx,sy,T,T);
          CX.globalAlpha=1;
        }
        // Overlay crosswalk stripes on road tiles touching sidewalk
        if(_adjSW && SP.tiles.crosswalk){
          CX.globalAlpha=0.80;
      ...
      // Lane centre dashes (only when no PNG loaded, to avoid double markings)
      if(!SP.tiles[1]){

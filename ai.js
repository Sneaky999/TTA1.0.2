...
function _aiGangsters(dt) {
  ...
  for (let i = gangs.length - 1; i >= 0; i--) {
    ...
    if (hpPct < AI_GANG_RETREAT_HP && homeDist > 55) {
      ...
      mvE(g,
        Math.cos(g.angle) * g.spd * 0.8 * dt,
        Math.sin(g.angle) * g.spd * 0.8 * dt);
    ...
    if (homeDist > maxRoam && pdd > 80) {
      ...
      g.angle = Math.atan2(g.homeY - g.y, g.homeX - g.x);
      mvE(g,
        Math.cos(g.angle) * g.spd * dt,
        Math.sin(g.angle) * g.spd * dt);
      ...
      g.wanderT -= dt;
      mvE(g,
        Math.cos(g.angle) * g.spd * 0.5 * dt,
        Math.sin(g.angle) * g.spd * 0.5 * dt);
    }
  }
}
...
function _aiCops(dt) {
  ...
  for (let i = cops.length - 1; i >= 0; i--) {
    ...
    if (PL.wanted > 0) {
      ...
      // ════════════════════════════════════════
      if (inGangZone) {
        // Evacuate gang zone immediately
        const wp = safePatrolPt(); c.px = wp.px; c.py = wp.py; c.patrolT = 15;
        ...
        if (isTileInGangZone(wpTX, wpTY, GANG_AVOID_MARGIN) ||
            Math.sqrt(wdx * wdx + wdy * wdy) < 32 ||
            c.patrolT <= 0) {
        ...
        c.angle = Math.atan2(wdy, wdx);
        mvE(c,
          Math.cos(c.angle) * gd.spd * 0.65 * dt,
          Math.sin(c.angle) * gd.spd * 0.65 * dt);
      }
    }
  }
}
    ...
    if (b.fp) {
      ...
      if (!gone) for (let j = gangs.length - 1; j >= 0; j--) {
        ...
        if (d2(b.x, b.y, g.x, g.y) < g.w * g.w) {
          ...
          bullets.splice(i, 1);
          if (g.hp <= 0) {
            PL.score += 80; PL.cash += 30; gangKills++;
            gangs.splice(j, 1);

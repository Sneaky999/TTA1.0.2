...
let _lastShotX      = undefined;  // world X of most recent player gunshot
let _lastShotY      = undefined;  // world Y of most recent player gunshot
...
function initAI() {
  ...
  _lastShotX     = undefined;
  _lastShotY     = undefined;
}
...
function panicNearby(x, y) {
  // Record shot position so nearby cops can witness it
  _lastShotX = x; _lastShotY = y;
  if (_panicCooldown > 0) return;
  _panicCooldown = AI_NPC_PANIC_COOLDOWN;
  const r2 = AI_NPC_PANIC_RADIUS * AI_NPC_PANIC_RADIUS;
  for (const n of npcs) {
    if (d2(n.x, n.y, x, y) < r2) {
      n.flee    = true;
      n.screamT = 1.8; // show panic icon for 1.8 s
    }
  }
}
...
function _aiNPCs(dt) {
  for (const n of npcs) {
    ...
    if (n.screamT > 0) {
      n.screamT -= dt;
      // Emit a small particle puff to visually show panic
      if (Math.random() < dt * 8) spawnPts(n.x, n.y - 10, '#fff', 1);
    }
...
function _aiGangsters(dt) {
  ...
  for (let i = gangs.length - 1; i >= 0; i--) {
    ...
    if (homeDist > maxRoam && pdd > 80) {
      ...
      // ── Provocation check ─────────────────────────────────────
      // Gangsters become hostile when shot at OR when wanted level ≥ 2.
      // This lets the player walk up to mission givers / zones in peace.
      if (!g.provoked && PL.wanted >= 2) g.provoked = true;
      // Cool down: if player goes clean they stand down
      if (g.provoked && PL.wanted === 0) g.provoked = false;

      if (g.provoked) {
        // ── Attack player (provoked) ─────────────────────────────

        // Alarm call: rally nearby allies when close
        ...
        // ── Unprovoked: idle wander near turf, watch but don't shoot ─
        if (!g.wanderT || g.wanderT <= 0) {
          g.angle   = Math.random() * Math.PI * 2;
          g.wanderT = 2.0 + Math.random() * 3.0;
        }
        g.wanderT -= dt;
        // Face player loosely when close — gives the sense of awareness
        if (pdd < 100) g.angle = Math.atan2(pdy, pdx);
        _gangSteerMove(g, g.angle, g.spd * 0.35, dt);
      }
...
function _aiCops(dt) {
  ...
  for (let i = cops.length - 1; i >= 0; i--) {
    ...
    // ── Provocation cooldown ──────────────────────────────────
    // Cops calm down after not seeing the player for a while,
    // unless there is an active wanted level.
    if (c.provoked && PL.wanted === 0) {
      c._calmT = (c._calmT || 0) + dt;
      if (c._calmT > 6) { c.provoked = false; c._calmT = 0; }
    } else {
      c._calmT = 0;
    }
    // Being in sight range of a shooting player re-provokes immediately
    if (!c.provoked && PL.wanted === 0) {
      if (typeof _lastShotX !== 'undefined' &&
          d2(c.x, c.y, _lastShotX, _lastShotY) < 260 * 260) {
        c.provoked = true;
        addWanted(1); // witnessing a crime adds one star
      }
    }

    const shouldChase = PL.wanted > 0 || c.provoked;

    if (shouldChase) {
      // ════════════════════════════════════════
      //  PRIORITY 1: Chase wanted / provoked player
      // ════════════════════════════════════════
    ...
    if (b.fp) {
      ...
      for (let j = npcs.length - 1; j >= 0; j--) {
        ...
        if (d2(b.x, b.y, n.x, n.y) < n.w * n.w) {
          n.hp -= b.dmg; n.flee = true; n.screamT = 2.2; spawnPts(n.x, n.y, '#f80', 4);
          // Spread panic to nearby civilians like a ripple
          const panicR2 = 280 * 280;
          for (const nearby of npcs) {
            if (nearby !== n && d2(nearby.x, nearby.y, n.x, n.y) < panicR2) {
              nearby.flee    = true;
              nearby.screamT = 1.2 + Math.random() * 0.8;
            }
          }
          if (b.spl > 0) explode(b.x, b.y, b.spl, b.dmg);
          bullets.splice(i, 1);
      ...
      if (!gone) for (let j = gangs.length - 1; j >= 0; j--) {
        ...
        if (d2(b.x, b.y, g.x, g.y) < g.w * g.w) {
          ...
          // Player shot this gangster → provoke them and nearby allies
          g.provoked = true;
          const alarmR2 = 200 * 200;
          for (const ally of gangs) {
            if (ally !== g && d2(ally.x, ally.y, g.x, g.y) < alarmR2) {
              ally.provoked = true;
            }
          }
          if (b.spl > 0) explode(b.x, b.y, b.spl, b.dmg);
      ...
      if (!gone) for (let j = cops.length - 1; j >= 0; j--) {
        ...
        if (d2(b.x, b.y, c.x, c.y) < c.w * c.w) {
          ...
          // Shot cop + nearby cops all become provoked instantly
          c.provoked = true;
          const alarmR2 = 320 * 320;
          for (const ally of cops) {
            if (ally !== c && d2(ally.x, ally.y, c.x, c.y) < alarmR2) {
              ally.provoked = true;
            }
          }
          if (b.spl > 0) explode(b.x, b.y, b.spl, b.dmg);
